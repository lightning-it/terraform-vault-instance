locals {
  active = {
    ...
    pki_roots     = { for k, v in var.vault_pki_roots : k => v if !try(v.absent, false) }
    pki_inters    = { for k, v in var.vault_pki_intermediates : k => v if !try(v.absent, false) }
    pki_roles     = { for k, v in var.vault_pki_roles : k => v if !try(v.absent, false) }
    ...
    transit_keys  = { for k, v in var.transit_secret_backend_key : k => v if !try(v.absent, false) }
    namespaces    = { for k, v in var.namespace : k => v if !try(v.absent, false) }
  }
